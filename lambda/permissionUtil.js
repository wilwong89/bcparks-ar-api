...
const SSO_ISSUER =
  process.env.SSO_ISSUER || "https://dev.oidc.gov.bc.ca/auth/realms/g7v0xlf4";
const SSO_JWKSURI =
  process.env.SSO_JWKSURI ||
  "https://dev.oidc.gov.bc.ca/auth/realms/g7v0xlf4/protocol/openid-connect/certs";
const INVALID_TOKEN = {
  decoded: false,
  data: null,
};
...
function verifySecret(tokenString, secret, callback, sendError) {
  jwt.verify(tokenString, secret, function (verificationError, decodedToken) {
    ...
    if (
      ...
      decodedToken &&
      decodedToken.resource_access["attendance-and-revenue"] &&
      decodedToken.resource_access["attendance-and-revenue"].roles
    ) {
